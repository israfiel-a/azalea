...
void utilities_stringSplitLines(char* string, char** storage, size_t *count) {
    ...
    while(*stringCopy != 0) {
        if(*stringCopy == '\n') {
            ...
            if(stringCopy - segmentBeginning != 0 && (*segmentBeginning != '/' && *(segmentBeginning + 1) != '/')) {
...
void utilities_numberToString(size_t number, size_t length, char* string) {
    for(size_t i = 0; i < length; i++) {
        switch(number % 10) {
            case 0:  string[length - i - 1] = '0'; break;
            case 1:  string[length - i - 1] = '1'; break;
            case 2:  string[length - i - 1] = '2'; break;
            case 3:  string[length - i - 1] = '3'; break;
            case 4:  string[length - i - 1] = '4'; break;
            case 5:  string[length - i - 1] = '5'; break;
            case 6:  string[length - i - 1] = '6'; break;
            case 7:  string[length - i - 1] = '7'; break;
            case 8:  string[length - i - 1] = '8'; break;
            default: string[length - i - 1] = '9'; break;
        }
        number /= 10;
    }
    string[length] = 0;
